// Cerebras provider (using Groq-compatible API with LangChain)
...
export class CerebrasProvider {
  ...
  /**
   * Initialize the Cerebras client (using Groq SDK with custom endpoint)
   */
  async initialize() {
    try {
      ...
      // Use ChatGroq with Cerebras endpoint
      ...
      console.log('✅ Cerebras provider initialized');
    } catch (error) {
      console.error('❌ Failed to initialize Cerebras provider:', error);
      throw error;
    }
  }
  ...
  async isAvailable() {
    ...
    return this.isInitialized && Boolean(process.env.CEREBRAS_API_KEY);
  }
  ...
  async generate(request) {
    ...
    try {
      ...
      trace = await observability.startTrace('cerebras-generate', {
        model: request.model,
        query: request.query,
      ...
      const model = request.model || 'llama3.1-70b';
      ...
      // Calculate cost
      const cost = estimateCost('cerebras', model, inputTokens, outputTokens);

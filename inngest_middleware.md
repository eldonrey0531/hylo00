...
TITLE: Self-hosted Inngest Gateway Endpoint Rewriting
DESCRIPTION: Demonstrates how to dynamically rewrite the Inngest gateway endpoint URL when self-hosting Inngest. This is useful for custom network configurations where the default gateway URL needs to be overridden.
...
TITLE: Common Inngest Endpoint Paths
DESCRIPTION: Lists common paths where the Inngest SDK might be served. These are useful when configuring the CLI or manually testing endpoints.
...
TITLE: Configuring Self-Hosted Inngest Server
DESCRIPTION: Instructions on how to configure Inngest SDKs to connect to a self-hosted Inngest server using environment variables.
...
- **INNGEST_EVENT_KEY** (string) - Required - Your Inngest event key.
- **INNGEST_SIGNING_KEY** (string) - Required - Your Inngest signing key.
- **INNGEST_DEV** (boolean) - Required - Set to `0` for self-hosted server.
- **INNGEST_BASE_URL** (string) - Required - The base URL of your self-hosted Inngest server.
```
...
DESCRIPTION: Example of an Inngest configuration file (`inngest.yaml` or `inngest.json`) specifying serve URLs, poll interval, and Redis/SQLite configurations.
...
### Example Configuration (`inngest.yaml` or `inngest.json`)
...
- **urls** (array of strings) - Required - List of Inngest serve endpoints for your application.
- **poll-interval** (integer) - Optional - The interval in seconds for polling.
- **redis-uri** (string) - Optional - URI for an external Redis server.
- **sqlite-dir** (string) - Optional - Directory for SQLite database files.
```
...
TITLE: Inngest Configuration File Example (YAML/JSON)
DESCRIPTION: An example of an Inngest configuration file, typically in YAML or JSON format. It specifies Inngest serve endpoints, polling intervals, and database URIs. This file can be used with the `--config` flag.
...
TITLE: Create Inngest Client
DESCRIPTION: Initializes the Inngest client with a unique application identifier. The `id` is a required string, recommended to be a hyphenated slug for your application.
...
TITLE: Initialize Inngest Client (Node.js)
DESCRIPTION: Demonstrates the basic instantiation of the Inngest client in a Node.js environment. It sets a unique application ID.
...
DESCRIPTION: Configuration details for using external services for queue and state store, specifically external Redis and PostgreSQL databases.
...
To use an external Redis server, set the `redis-uri` flag to the Redis server URI in your configuration file or use the `INNGEST_REDIS_URI` environment variable.

### External Postgres database

To use an external Postgres database, set the `postgres-uri` flag to your Postgres connection string URI in your configuration file or use the `INNGEST_POSTGRES_URI` environment variable.

### Parameters

- **redis-uri** (string) - Required (if using external Redis) - URI for the external Redis server.
- **postgres-uri** (string) - Required (if using external Postgres) - Connection string URI for the external PostgreSQL database.
```
...
DESCRIPTION: Explains how to use environment variables to configure the Inngest server, mirroring command-line flag options.
...
CODE:
```
## Inngest Server Environment Variables

### Description
Demonstrates how to configure the Inngest server using environment variables. Any command-line flag can be set by converting it to uppercase, replacing hyphens with underscores, and prefixing with `INNGEST_`.
...
### Parameters
#### Environment Variables
- **INNGEST_EVENT_KEY**: Corresponds to `--event-key` flag.
- **INNGEST_SIGNING_KEY**: Corresponds to `--signing-key` flag.
- **INNGEST_PORT**: Corresponds to `--port` flag.
- **INNGEST_HOST**: Corresponds to `--host` flag.
- **INNGEST_SDK_URL**: Corresponds to `--sdk-url` flag.
- **INNGEST_SQLITE_DIR**: Corresponds to `--sqlite-dir` flag.
- **INNGEST_REDIS_URI**: Corresponds to `--redis-uri` flag.
- **INNGEST_POSTGRES_URI**: Corresponds to `--postgres-uri` flag.
- **INNGEST_POSTGRES_MAX_IDLE_CONNS**: Corresponds to `--postgres-max-idle-conns` flag.
- **INNGEST_POSTGRES_MAX_OPEN_CONNS**: Corresponds to `--postgres-max-open-conns` flag.
- **INNGEST_POSTGRES_CONN_MAX_IDLE_TIME**: Corresponds to `--postgres-conn-max-idle-time` flag.
- **INNGEST_POSTGRES_CONN_MAX_LIFETIME**: Corresponds to `--postgres-conn-max-lifetime` flag.
- **INNGEST_POLL_INTERVAL**: Corresponds to `--poll-interval` flag.
- **INNGEST_RETRY_INTERVAL**: Corresponds to `--retry-interval` flag.
- **INNGEST_QUEUE_WORKERS**: Corresponds to `--queue-workers` flag.
- **INNGEST_TICK**: Corresponds to `--tick` flag.
- **INNGEST_CONNECT_GATEWAY_PORT**: Corresponds to `--connect-gateway-port` flag.
- **INNGEST_NO_UI**: Corresponds to `--no-ui` flag.
- **INNGEST_JSON**: Corresponds to `--json` flag.
- **INNGEST_VERBOSE**: Corresponds to `--verbose` flag.
- **INNGEST_LOG_LEVEL**: Corresponds to `--log-level` flag.

### Request Example
```bash
export INGEST_EVENT_KEY=<YOUR_EVENT_KEY>
export INGEST_SIGNING_KEY=<YOUR_SIGNING_KEY>
inngest start
```

### Response
N/A (Server starts using environment variables for configuration)

### Error Handling
- Ensure environment variables are correctly set before starting the server.
```
...
TITLE: Create Inngest Client (v3 vs v2)
DESCRIPTION: Demonstrates the required change from 'name' to 'id' when instantiating an Inngest client. The 'id' is now mandatory for client instantiation.
...
DESCRIPTION: A Docker Compose configuration example for setting up Inngest along with PostgreSQL and Redis services.
...
This example demonstrates how to set up Inngest with PostgreSQL and Redis using Docker Compose.
...
- **inngest**: The Inngest service container.
  - **Ports**: 8288 (HTTP), 8289 (gRPC)
  - **Environment Variables**: `INNGEST_EVENT_KEY`, `INNGEST_SIGNING_KEY`, `INNGEST_POSTGRES_URI`, `INNGEST_REDIS_URI`
...
TITLE: Configuring Inngest SDK for Self-Hosted Server (Node.js)
DESCRIPTION: Demonstrates how to configure Node.js applications to connect to a self-hosted Inngest server using environment variables. It sets essential keys and the base URL for the Inngest server.
...
TITLE: Configure Inngest Dev Server (JSON/YAML)
DESCRIPTION: Provides an example of configuring the Inngest CLI using a configuration file (inngest.json or inngest.yaml). This file can specify multiple SDK URLs and disable auto-discovery, useful for complex setups.
...
TITLE: Docker Compose for Inngest with PostgreSQL and Redis
DESCRIPTION: A Docker Compose configuration file setting up Inngest, PostgreSQL, and Redis services. It defines environment variables for database and Redis URIs, network configurations, and health checks for each service.
...
TITLE: Inngest v2 Stacking Event Schema Sources
DESCRIPTION: Demonstrates how to combine multiple event schema definitions in Inngest v2 by chaining methods like fromRecord, fromUnion, and fromZod. This allows for modular and comprehensive schema definitions.
...
TITLE: Inngest Client Initialization
DESCRIPTION: Demonstrates how to initialize the Inngest client with essential configuration parameters like the application ID.
...
### Description
This section shows how to create an instance of the Inngest client, which is used to configure your application and send events.
...
### Parameters
#### Request Body
- **api_base_url** (str) - optional - Override the default base URL for our REST API (`https://api.inngest.com/`). See also the `INNGEST_EVENT_API_BASE_URL` environment variable.
- **app_id** (str) - required - A unique identifier for your application. We recommend a hyphenated slug.
- **env** (str) - optional - The environment name. Required only when using Branch Environments.
- **event_api_base_url** (str) - optional - Override the default base URL for sending events (`https://inn.gs/`). See also the `INNGEST_EVENT_API_BASE_URL` environment variable.
- **event_key** (str) - optional - An Inngest event key. Alternatively, set the `INNGEST_EVENT_KEY` environment variable.
- **is_production** (bool) - optional - Whether the SDK should run in production mode. See also the `INNGEST_DEV` environment variable.
- **logger** (logging.Logger | logging.LoggerAdapter) - optional - A logger object derived from `logging.Logger` or `logging.LoggerAdapter`. Defaults to using `logging.getLogger(__name__)` if not provided.
- **middleware** (list) - optional - A list of middleware to add to the client. Read more in our middleware docs.
- **signing_key** (str) - optional - The Inngest signing key. Alternatively, set the `INNGEST_SIGNING_KEY` environment variable.
...
### Response
#### Success Response (200)
An initialized Inngest client object.

#### Response Example
```python
# The Inngest client object is now configured and ready to use.
pass
```
```
...
TITLE: Initialize Inngest Client (TypeScript)
DESCRIPTION: Instantiate the Inngest client with your application's ID. This client will be used to send events. It's recommended to do this once and export it for use throughout your application.
...
TITLE: Inngest v3 Environment Variables
DESCRIPTION: Details on new and removed environment variables for configuring Inngest in v3.
...
### Added
- **INNGEST_BASE_URL**: Sets the URL for communicating with Inngest (e.g., `http://localhost:8288`).
- **INNGEST_SERVE_HOST**: Sets the `serveHost` option (e.g., `https://www.example.com`).
- **INNGEST_SERVE_PATH**: Sets the `servePath` option (e.g., `/api/inngest`).
- **INNGEST_LOG_LEVEL**: Sets the logging level (e.g., `"fatal" | "error" | "warn" | "info" | "debug" | "silent"`).
- **INNGEST_STREAMING**: Configures streaming behavior (e.g., `"allow" | "force" | "false"`).

### Removed
- **INNGEST_API_BASE_URL**: Use `INNGEST_BASE_URL` instead.
- **INNGEST_DEVSERVER_URL**: Use `INNGEST_BASE_URL` instead.
```
...
TITLE: Inngest v2 Event Schema Typing with fromZod
DESCRIPTION: Shows how to integrate Zod schemas for event typing in Inngest v2 using the fromZod method. This enables leveraging Zod's validation capabilities for defining event data structures.
...
TITLE: Catch and Handle Step Errors with try/catch
DESCRIPTION: Demonstrates how to use a try/catch block to catch `StepError` and recover by using a backup API. This feature is available from Inngest TypeScript SDK v3.12.0+.
...
inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    try {
      ...
      data = await step.run('use-backup-weather-data', async () => {
...
TITLE: Inngest Server Configuration Options
DESCRIPTION: Lists all available command-line flags for configuring the Inngest server, including network, persistence, and logging settings.
...
CODE:
```
## Inngest Server Configuration Flags

### Description
Provides a comprehensive list of command-line flags to configure the Inngest server's behavior, including network settings, persistence options, and logging levels.
...
- **--port** (string, -p) - Inngest server port (default: "8288").
- **--sdk-url** (string, -u) - App serve URLs to sync (e.g., http://localhost:3000/api/inngest).
...
- **--redis-uri** (string) - Redis server URI for external queue and run state.
- **--postgres-uri** (string) - PostgreSQL database URI for configuration and history persistence.
...
- **--log-level** (string, -l) - Set the log level. One of: trace, debug, info, warn, error (default: "info").

### Request Example
```bash
inngest start --port 8000 --sqlite-dir ./data
```

### Response
N/A (Server starts with configured options)

### Error Handling
- Invalid flag values may result in errors during server startup.
```
...
TITLE: Create Webhook Intent
DESCRIPTION: Generates a URL for creating a webhook intent. Users are redirected to this URL to approve and create a new webhook URL.
...
CODE:
```
## POST /intent/create-webhook

### Description
Creates a URL that facilitates the creation of a webhook intent, allowing users to approve and configure a new webhook URL for Inngest.
...
### Parameters
#### Query Parameters
- **name** (string) - required - The name of the webhook intent, used for identification and as a slug prefix for events.
- **redirect_uri** (string) - required - The URL to redirect the user back to after approval, which will include the new webhook URL as a query parameter.
...
### Response
#### Success Response (200)
- **url** (string) - The newly created webhook URL, provided as a query parameter in the redirect.
...
TITLE: Building Webhook Integrations
DESCRIPTION: Introduces Inngest's webhook intents feature, which facilitates building integrations with various applications. It directs users to further resources for detailed guidance.
...
Inngest has a feature called **webhook intents** that allows you to build webhook integrations with any application. Read more about building webhook integrations here: [Link to further documentation on webhook intents]
...
TITLE: Inngest v3 Client Configuration
DESCRIPTION: Details on new and removed configuration options for the Inngest client in v3.
...
#### Added
- **baseUrl** (string): Sets the URL for communicating with Inngest. Synonymous with `INNGEST_BASE_URL`.
...
TITLE: Inngest Server Start Command
DESCRIPTION: This example shows how to start the Inngest server using command-line flags for event and signing keys.
...
CODE:
```
## Inngest Server Start

### Description
Starts the Inngest server with specified event and signing keys.
...
### Parameters
#### Command-line Flags
- **--event-key** (string) - Required - The event key used by apps to send events to the server.
- **--signing-key** (string) - Required - The signing key used to sign and validate data between the server and apps.
...
### Error Handling
- Invalid keys may prevent the server from starting.
```
...
TITLE: Environment Variable Configuration
DESCRIPTION: Demonstrates how to set Inngest server configuration options using environment variables. This method allows overriding command-line flags and is useful for deployment environments.
...
CODE:
```
# Example: Set the port using an environment variable
export INNNGEST_PORT=8288

# Example: Set event and signing keys via environment variables
export INGEST_EVENT_KEY=<YOUR_EVENT_KEY>
export INGEST_SIGNING_KEY=<YOUR_SIGNING_KEY>
```
...
TITLE: Initialize Inngest Client with TypeScript Types
DESCRIPTION: Sets up the Inngest client with custom event schemas for strong typing. This involves defining event types and using `EventSchemas` to infer them.
...
TITLE: Inngest Start Command Help
DESCRIPTION: Displays comprehensive help information for the `inngest start` command, detailing all available options for configuring the server's behavior, including ports, data stores, and sync intervals.
...
TITLE: Configure Inngest ESLint Plugin (Recommended)
DESCRIPTION: Configures ESLint to use the Inngest plugin with its recommended settings. This enables a set of predefined rules to enforce Inngest best practices.

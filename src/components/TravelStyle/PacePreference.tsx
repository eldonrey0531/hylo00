// src/components/TravelStyle/PacePreference.tsx
import React, { useCallback, useEffect } from 'react';
import { BaseStyleFormProps, TRAVEL_PACE_OPTIONS, TravelPace } from './types';

const PacePreference: React.FC<BaseStyleFormProps> = ({
  styleData,
  onStyleChange,
  validationErrors,
  onValidation,
}) => {
  const currentPace = styleData.pace;

  useEffect(() => {
    // Validate on mount and when pace changes
    if (onValidation) {
      const isValid = Boolean(currentPace);
      const errors = isValid ? [] : ['Please select your preferred travel pace'];
      onValidation('pace', isValid, errors);
    }
  }, [currentPace, onValidation]);
  ...
  const hasError = validationErrors?.['pace'];
  ...
  return (
    <div className="bg-form-box rounded-[36px] p-6 border-3 border-gray-200">
      <div className="mb-4">
        ...
        <p className="text-primary font-bold font-raleway text-xs">How do you prefer to travel?</p>
        {hasError && (
          <p

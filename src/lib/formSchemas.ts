...
export const tripDetailsSchema = z
  .object({
    ...
    departDate: z
      .string()
      .optional()
      .refine((date) => !date || date.trim() === '' || !isNaN(Date.parse(date)), 'Invalid date format'),
  ...
  .superRefine((data, ctx) => {
    ...
    // Conditional validation for dates based on flexibleDates
    if (data.flexibleDates) {
      // When flexible dates, plannedDays is required
      if (!data.plannedDays || data.plannedDays < 1) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: 'Planned days is required when using flexible dates',
          path: ['plannedDays'],
        });
      }
    } else {
      // When fixed dates, departDate and returnDate are required
      if (!data.departDate || data.departDate.trim() === '') {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: 'Departure date is required when not using flexible dates',
          path: ['departDate'],
        });
      }
      if (!data.returnDate || data.returnDate.trim() === '') {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: 'Return date is required when not using flexible dates',
          path: ['returnDate'],
        });
      }
    }
